...
class logs_emails_base extends lib_base{

    function __construct()
    {
    	$this->appTable 		= 'logs_emails_view';
    	$this->appRecordId		= 'record_id';    	
    	$this->appRecordName	= 'record_name';    	
    	$this->appOrderBy		= 'record_name';    	
    	$this->appAscDesc		= 'asc';    	
    	$this->appName 			= 'logs_emails';
    	$this->appNameSingular	= 'logs_record';    	
    	$this->appLabel 		= 'List Records';
    	$this->appLabelSingular = 'List Record';    
    	$this->appXSL	 		= 'logs.records_base';
    	$this->appRole 			= 'logs';
	...
	function search(  $args = null)
    {
    	...
    	{
    		$list = getOneAssocArray('select list_id from logs_lists order by list_name limit 1');
    		if( $list['list_id'] == null )
    		{
    			messagebox( lang('No list, please create a list'), ERROR);
    			return execMethod( 'logs_lists.base.start');
    		}
    ...
    function bulk_remove( $args)
    {
		...
		switch( $args['bulk_id'] )
		{
			...
			case 'Page':				 
				...
				foreach( $selected as $key => $val)
				{
 					if( deleteRowWhere( 'logs_emails', 'where list_id = "'.$_SESSION[$this->appName]['list_id'].'" and record_id = "'.
						$val.'"', false) == true )
						$_SESSION[$this->appName]['countMax']--;
				}	
				break;	
				...
				foreach( $selected as $key => $val)
				{
						...
						$GLOBALS['appshore']->rbac->checkDataPermissions( $this->appRole, $related['table_name'], $related['field_name'], $val ) == true )
					{
						...
						updateRow( $related['table_name'], $related['field_name'], $record, false);
	 					...
	 					updateRowWhere( 'logs_emails', 'where list_id = "'.$_SESSION[$this->appName]['list_id'].
