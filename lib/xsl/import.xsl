...
<xsl:template name="import_upload">
	...
	<xsl:if test = "import/isDuplicate = '1'">		
		<div class="clearboth fieldLabelContainer" style="padding-top: 10px;">
			<div class="fieldLabel">
				<xsl:value-of select="php:function('lang','Allow duplicate records')"/>
			</div>
			<div class="fieldText">
				<input type="checkbox" name="duplicate" >
					<xsl:attribute name="onclick">is_boxchecked(this)</xsl:attribute>
					<xsl:attribute name="value"><xsl:value-of select="import/duplicate" /></xsl:attribute>
					<xsl:if test="import/duplicate = 'Y'">
						<xsl:attribute name="checked"/>
					</xsl:if>					
				</input>
			</div>
	...
	<xsl:if test = "import/isRelated = '1'">
		<div class="clearboth fieldLabelContainer" style="padding-top: 10px;">
			<div class="fieldLabel">
