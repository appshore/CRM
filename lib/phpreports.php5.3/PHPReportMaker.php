	...
	class PHPReportMaker {
		...
		function PHPReportMaker() {
			...
			*/			
			...
			if(is_null($this->_oProc))
				new PHPReportsError('There is no XSLT processor available.','noxsltprocessor.php');
				
			// check path stuff
			if(is_null(getPHPReportsFilePath()))
				new PHPReportsError('I can not find where the classes are.','nopath.php');
		}
		...
		******************************************************************************/
		function run() {
			...
			$aParm["user"] 		= $GLOBALS['appshore_data']['server']['db_user']; //$this->sUser;	// set user
			$aParm["pass"] 		= $GLOBALS['appshore_data']['server']['db_pass']; //$this->sPass;	// set password
			$aParm["conn"] 		= $this->sCon;	// set connection name
			$aParm["interface"] = $this->sDataI;	// set database interface
			$aParm["database" ] = $GLOBALS['appshore_data']['server']['db_name']; //$this->sDatabase;	// set database
