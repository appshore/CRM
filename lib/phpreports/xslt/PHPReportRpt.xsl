...
<xsl:template name="DATACON">
	...
	if(!file_exists($sIfFile))
		new PHPReportsError("No database interface '$sIf' available.");
	include_once $sIfFile; 

	// open database connection
	if(is_null($this->_oCon))
		$oCon = @PHPReportsDBI::db_connect(Array($sUser,$sPass,$sConn,$sData)) or new PHPReportsError("Connection refused.","connectionrefused.php");
	else 
		$oCon = $this->_oCon;

	$this->_aBench["sql_start"] = time();
	$oStmt= @PHPReportsDBI::db_query($oCon,trim($sSQL)) or new PHPReportsError("Error executing query.","queryerror.php");
