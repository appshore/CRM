...
class DOMi
    {
        var $ItemSuffix = "_item";              ///BMI: 18/12/08 < @var string the text that will be attached to the end of a prefix when it applies to an array
        var $ListSuffix = "";              		///BMI: 18/12/08 < @var string the text that will be attached to the end of a prefix when it applies to an array
                ...
                if($this->ValidPrefix($Prefix) === false)
                    {
                        ...
                        // BMI: we just ignore the invalid prefix
                        return false;
                    }
                else
                    {
                ...
                if(is_string($Data) || is_numeric($Data) || is_int($Data))
                    {
                        if(file_exists($Data) && $this->IsValidXml($Data))
                            ...
                            {
                                if($Data != htmlspecialchars($Data))	// BMI: added to handle some special cases
                ...
                elseif(is_array($Data) && strtolower($Prefix) != 'attributes')
                    {
                        ...
                        foreach ($DataI as $Value)
                            {
                                $this->AttachToXml($Value, $Prefix.$this->ItemSuffix, $NewNode, $this->SiblingsExist($Data));
                            }
