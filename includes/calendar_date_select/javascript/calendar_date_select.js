...
Date.months2 = $w('January February March April May June July August September October November December');
...
Date.parseFormattedString = function(string) { 
	...
	if( isNaN(date) )
	{
		for ( var i = 11; i > -1 ; i--)
		{
			if( string.search(Date.months[i]) != -1 )
				return new Date( string.replace(Date.months[i], Date.months2[i]));
		}
	}
	return date;
};
Math.floor_to_interval = function(n, i) { return Math.ceil(n/i) * i;}
...
CalendarDateSelect.prototype = {
  ...
  initButtonsDiv: function()
  {
    ...
    if (this.options.get("buttons")) {
      buttons_div.build("div", {innerHTML: ""}); //BMI: buttons_div.build("span", {innerHTML: "&#160;"});
