...
function bmhBodyRules($body,$structure,$debug_mode=false) {
   ...
   * (xxxxx@yourdomain.com)
   */
  elseif (preg_match ("/This user doesn't have a yahoo.com account.*\((\S+@\S+\w)\)/is",$body,$match)) {
   ...
   * rule: mailbox unknown;
   * sample:
   * - Invalid mailbox: xxxxx@yourdomain.com
   */
  elseif (preg_match ("/ - Invalid mailbox: (\S+@\S+\w)/i",$body,$match)) {
    $result['rule_cat']    = 'unknown';
    $result['rule_no']     = '0238';
    $result['email']       = $match[1];
  }
  ...
  if (empty($result['email'])) {
    ...
    switch ($action) {
      case 'failed':
         ...
         */
        elseif (preg_match ("/(?:alias|account|recipient|address|email|mailbox|user).*(?:n't|not)\s*exist/is",$diag_code)) {
          $result['rule_cat']    = 'unknown';
          $result['rule_no']     = '0205';
        }
        /* rule: unknown
        ...
        elseif ((preg_match ("/Host unknown/i",$dsn_msg)) ||
                (preg_match ("/Host not found/i",$dsn_msg))) {
          $result['rule_cat']    = 'dns_unknown';
          $result['rule_no']     = '0140';
        }
        /* rule: dns_unknown
