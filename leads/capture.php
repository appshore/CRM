...
$user = getOneAssocArray( 'select users.user_id, users.full_name, users.email, company.url from users, company where users.user_id = company.main_user_id');	
...
else
{
	if( $_REQUEST['emailAlert'] == 'yes' )
	{
		...
		$mail->FromName = lang('%(BRAND)s - Web lead capture', array('BRAND'=>BRAND));
	
		$mail->Subject 	= lang('New lead from %(leadFrom)s: %(account_name)s, %(full_name)s',array('leadFrom'=>$leadFrom,'account_name'=>$lead['account_name'],'full_name'=>$lead['full_name']));

		// HTML message
		$sep = "<BR/>";		
 		$mail->Body  = lang('Dear %(full_name)s,', array('full_name'=>$user['full_name'])).$sep.$sep;
		$mail->Body .= lang('New lead from %(leadFrom)s.',array('leadFrom'=>$leadFrom)).$sep.$sep;
        ...
        $emailTo= $capture->cleanup($_REQUEST['emailTo'] ? $_REQUEST['emailTo'] : $user['email']); 	
