...
<xsl:template match="APP">
    <table height="300px" width="100%" cellSpacing="0" cellPadding="0"  border="0" >
		<tr width="100%" valign="top">
			<td >
				<xsl:choose>
					<xsl:when test="action/reports = 'beyondforestry.start_popup' ">
					    <xsl:call-template name='activity_start'/>
					</xsl:when>
				</xsl:choose>
			</td>
		</tr>
...
<xsl:template name='activity_start'>
...
<![CDATA[
	...
	<form id='nuc_date' name='nuc_date' method='post'>
		<input type='hidden' name='op' value='reports.beyondforestry.start_popup'/>
