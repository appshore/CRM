...
<xsl:template name="yearly_filter">
	<form id="yearly_filter" name="yearly_filter" method="post">
		<input type="hidden" name="op" id="op" value="forecasts.base.yearly"/>			
    ...
    <div class="clearboth">
        <xsl:call-template name="customSearchFooter">
			<xsl:with-param name="thisForm" select="'yearly_filter'"/>
		</xsl:call-template>&#160;
