...
<xsl:template name="beta">
	...
	<div>			
		<br/>
		<xsl:value-of select="php:function('lang','AppShore is currently running in testing mode for the next few weeks.')"/><br/><br/>
		<xsl:value-of select="php:function('lang','We do not feel it right to charge you during a beta test.')"/>&#160;				
		<xsl:value-of select="php:function('lang','So please enjoy AppShore freely until the end of the period.')"/><br/><br/>				
		...
		<xsl:value-of select="php:function('lang','Thank you for helping us to make AppShore better!')"/><br/><br/>
		<xsl:value-of select="php:function('lang','The AppShore Team')"/>										
	</div>
</xsl:template>
...
<xsl:template name="close_confirmed">
	...
	<h3 style="text-align:center;padding:3em">		
		<xsl:value-of select="php:function('lang','Your AppShore account is now closed')"/><br/><br/>
		<xsl:value-of select="php:function('lang','The AppShore Team')"/>										
	</h3>
</xsl:template>
...
<xsl:template name="close">
	...
	<form name="close" method="post">
		...
		<div style="height:2em;line-height:2em;vertical-align:middle">
			<xsl:value-of select="php:function('lang','Yes, I confirm that I want to close my AppShore account')"/>&#160;
		...
		<input type="submit" class="formBarButton" name="close" onclick="document.close.key.value='Close'">
	        <xsl:attribute name="value">
	            <xsl:value-of select="php:function('lang','Close AppShore')"/>
	        </xsl:attribute>
	    </input>
...
<xsl:template name="upgrade_success">
	...
	<div>			
		<br/>
		<xsl:value-of select="php:function('lang','You have successfully modified your AppShore subscription.')"/><br/><br/>
...
<xsl:template name="upgrade_cancel">
	...
	<div>
		...
		<xsl:value-of select="php:function('lang','If you did not intend to cancel your order, or you are having difficulty in the ordering process, please contact AppShore at')"/>&#160;
		<xsl:call-template name="mailto">
			<xsl:with-param name="email"><xsl:value-of select="/APPSHORE/API/support_email"/></xsl:with-param>
			<xsl:with-param name="name"><xsl:value-of select="/APPSHORE/API/brand"/> Team</xsl:with-param>
...
<xsl:template name="upgrade">
	...
	<div class="helpmsg">
		...
		<br/><xsl:value-of select="php:function('lang','If you wish to pay by another means, please contact')"/>&#160;
		<xsl:call-template name="mailto">
	...
	<div >
		<xsl:value-of select="php:function('lang','The current edition is')"/>&#160;
		<xsl:value-of select="subscription/current/edition_name"/>
	</div>
	<div style="font-weight:bold">	
		<xsl:value-of select="php:function('lang','The new edition will be')"/>&#160;
	...
	<xsl:if test="subscription/current/due_date">
		<div>	
			<xsl:value-of select="php:function('lang','The current due date is')"/>&#160;
			<xsl:value-of select="subscription/current/due_date"/>
		</div>
    </xsl:if>				
	<div>	
		<xsl:value-of select="php:function('lang','I want to add')"/>&#160;
		...
		&#160;<xsl:value-of select="php:function('lang','month(s)')"/>
	</div>
	<xsl:if test="subscription/new/due_date">
		<div style="font-weight:bold">
			<xsl:value-of select="php:function('lang','The new due date will be')"/>&#160;
	...
	<div>
		<xsl:value-of select="php:function('lang','The current number of users is')"/>&#160;
		<xsl:value-of select="/APPSHORE/APP/subscription/current/users_quota"/>
	</div>
	<div style="font-weight:bold">
		<xsl:value-of select="php:function('lang','The new total number of users will be')"/>&#160;
		<input type="text" size="4" name="users_quota" value="{subscription/new/users_quota}" onchange="document.upgrade.key.value='Update';document.upgrade.submit();"/>
	</div>
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<div style="font-weight:bold;margin-top:20px">
				<xsl:value-of select="php:function('lang','The amount to pay is')"/>&#160;
				...
				<xsl:otherwise>	    
					...
					<div style="margin-top:20px">
						<input type="submit" class="formBarButton" name="GoogleWallet" onclick="document.upgrade.key.value=this.name">
							<xsl:if test="not(subscription/new/updated)"><xsl:attribute name="disabled" value="true" /></xsl:if>
							<xsl:attribute name="value"><xsl:value-of select="php:function('lang','Subscribe with Google Wallet')"/></xsl:attribute>
						</input>&#160;
						<input type="submit" class="formBarButton" name="Paypal" onclick="document.upgrade.key.value=this.name">
							<xsl:if test="not(subscription/new/updated)"><xsl:attribute name="disabled" value="true" /></xsl:if>
							<xsl:attribute name="value"><xsl:value-of select="php:function('lang','Subscribe with PayPal')"/></xsl:attribute>
						</input>
					</div>
				</xsl:otherwise>
			</xsl:choose>
...
<!--				<input type="image" name="Google Wallet"-->
<!--					src="http://sandbox.google.com/checkout/buttons/checkout.gif?merchant_id={google/merchant_id}&amp;w=180&amp;h=46&amp;style=white&amp;variant=text&amp;loc=en_US"-->
<!--					height="46" width="180"/>				-->
				<input type="image" name="Google Wallet" alt="Fast checkout through Google"
					src="https://checkout.google.com/checkout/buttons/checkout.gif?merchant_id={google/merchant_id}&amp;w=180&amp;h=46&amp;style=white&amp;variant=text&amp;loc=en_US"
					height="46" width="180"/>				
			</td>
		</tr>
		<tr>
			<td colspan="2">
				<br/>
				<xsl:value-of select="php:function('lang','Please go back to your AppShore account after your payment to complete the process.')"/><br/><br/>
				<xsl:value-of select="php:function('lang','Thank you for your business!')"/><br/><br/>				
				<xsl:value-of select="php:function('lang','The AppShore Team')"/>				
			</td>
		</tr>				
