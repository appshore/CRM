    ...
    <table width="100%" cellSpacing="0" cellPadding="0"  border="0" >
		<tr width="100%" valign="top">
            ...
            <td id="right_panel" class="right_panel">
			    <xsl:call-template name="administration_start"/>
			</td>
		</tr>
	</table>
</xsl:template> 

<xsl:template name="administration_start">
	<xsl:call-template name="headerListForm">
		<xsl:with-param name="appLabel">Administration</xsl:with-param>
	</xsl:call-template>	
    <div>
		<xsl:value-of disable-output-escaping="yes" select="php:function('lang','administration.base.start')"/>
	</div>
</xsl:template>
