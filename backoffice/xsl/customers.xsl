            ...
            <td id="right_panel" class="right_panel">
				<xsl:choose>
					<xsl:when test="action/node()[name() = /APPSHORE/APP/recordset/appName] = 'search'">
				    	...
				    	<xsl:call-template name="customers_grid">
				    		<xsl:with-param name="appName" select="/APPSHORE/APP/recordset/appName"/>
				    		<xsl:with-param name="appLabel" select="/APPSHORE/APP/recordset/appLabel"/>
				    	...
				    	<xsl:call-template name="custom_bulk">
				    		<xsl:with-param name="appName" select="/APPSHORE/APP/recordset/appName"/>
				    		<xsl:with-param name="nodeName" select="/APPSHORE/APP/recordset/appName"/>	    		
				    		<xsl:with-param name="selectedForm" select="'customers_grid'"/>				    		
				    	</xsl:call-template>
					</xsl:when>
					<xsl:otherwise>
...
<xsl:template name="customers_grid">
	...
	<xsl:variable name="formName" select="'customers_grid'"/>							
		...
		<table class="resizable searchResultsTable" border="0" cellspacing="0" cellpadding="0">
			...
			<xsl:for-each select = "/APPSHORE/APP/node()[name() = $nodeName]/node()[name() = $items]">
				...
				<tr class="{record_class}" onMouseOver="this.className ='searchResultsSelected'" onMouseOut="this.className ='{record_class}'">
					...
					<td class="searchResultsIcons" >		
						<a href="{$baseurl}&amp;op=backoffice.users_base.search&amp;{$recordId}={$recordId_value}" target="bo_users" title="View users of this customer">
...
<xsl:template match="/APPSHORE/API/Plugins" mode="ViewButtons">
	...
	<xsl:call-template name="setupButton">
...
<xsl:template name="setupButton">
	<xsl:param name="appName"/>
	<xsl:param name="thisForm"/>	
	<input type="submit" class="formBarButton" name="Setup" id="Setup" onclick="document.{$thisForm}.key.value=this.name">
        <xsl:attribute name="title">
            <xsl:value-of select="php:function('lang','Setup the instance')"/>
        </xsl:attribute>
