	...
	function swapRow(item, updown) 
	{
		if( updown == 'DN')
			if( DragUtils.nextItem(item))
				DragUtils.swap( DragUtils.nextItem(item), item);
		if( updown == 'UP')
			if( DragUtils.previousItem(item))
				DragUtils.swap( item, DragUtils.previousItem(item));
	};			
	...
	function saveForm( thisForm, appName) 
	{
		...
		if( $('used') )
			fields = serializeFields($('used').getElementsByTagName('div'));
		else
		{
			var items = $('layout').getElementsByTagName('ul');
        	for (var i = 0; items.length > i ; i++) 
	       		if( items[i].id )
	       		{
                	if(i)
                    	fields += ';';           
					fields += items[i].id+':'+serializeFields($(items[i].id).getElementsByTagName('li'));
				}
		}
